...
int		ft_env(t_msh *msh)
{
	...
	if (!msh->input[1])
		return (ft_print_env_lst(msh->envp));
	...
	ret = env_creat_lst_env(msh->input, &(fresh.envp), ac);
	if (ac == 1)
		ft_join_list(env_list_cpy(msh->envp), &(fresh.envp), 1);
	ac = ret;
	tmp = ft_tab_to_str((msh->input+ac));
	if (!(msh->input[ac]))
		ft_print_env_lst(fresh.envp);
